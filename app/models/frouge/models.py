...
def convolutional(
    y, filters, kernel_size, batch_norm=False, activation="relu", strides=(1, 1)
):
...
def residual(y, filters, kernel_size, batch_norm, strides):
    shortcut = convolutional(
        y, filters, kernel_size=(1, 1), batch_norm=False, strides=strides
    )
    shortcut = convolutional(
        shortcut, filters, kernel_size=(1, 1), batch_norm=False, strides=strides
    )

    y = convolutional(
        y, filters, kernel_size=kernel_size, batch_norm=batch_norm, strides=strides
    )
