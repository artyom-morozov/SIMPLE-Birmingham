...
class FlammeRougeEnv(gym.Env):
    ...
    def __init__(self, verbose=False, manual=False):
        ...
        self.observation_space = gym.spaces.Box(
            ...
            (
                MAX_BOARD_SIZE,
                3,
                (MAX_CODE + 2 * self.n_players)
                + card_types * self.n_players
                + 2 * card_types
                + self.action_space.n,
            ),
        )
    ...
    def observation(self):
        ...
        board_array = np.append(
            board_array, np.zeros((board_array.shape[0], 3, 2 * self.n_players)), axis=2
        )
        # add current player position info
        board_array[
            self.current_player.r_position.col,
            self.current_player.r_position.row,
            MAX_CODE,
        ] = 1
        board_array[
            self.current_player.s_position.col,
            self.current_player.s_position.row,
            MAX_CODE + 1,
        ] = 1
        ...
        for player_num in range(self.n_players):
            if player_num != self.current_player_num:
                board_array[
                    self.board.players[player_num].r_position.col,
                    self.board.players[player_num].r_position.row,
                    MAX_CODE + 2 + 2 * i,
                ] = 1
                board_array[
                    self.board.players[player_num].s_position.col,
                    self.board.players[player_num].s_position.row,
                    MAX_CODE + 3 + 2 * i,
                ] = 1
                i += 1
        obs = board_array
        ...
        deck = np.add(
            self.current_player.r_played.array(), self.current_player.s_played.array()
        )
        ...
        deck = np.add(
            self.current_player.r_discard.array(), self.current_player.s_discard.array()
        )
        deck = np.expand_dims(deck, [0, 1])
        deck = np.repeat(deck, MAX_BOARD_SIZE, axis=0)
        deck = np.repeat(deck, 3, axis=1)
        ...
        # add player's hand
        hand = np.add(
            self.current_player.r_hand.array(), self.current_player.s_hand.array()
        )
    ...
    def legal_actions(self):
        ...
        if self.phase == 2:
            ...
            for i in range(MAX_START_SPACES):
                col = i // 3
                row = i % 3
                if self.board.get_cell(col, row) == CS and self.board.is_empty(
                    col, row
                ):
    ...
    def score_game(self):
        ...
        scores = [
            (
                max(
                    (p.r_position.col * 3 - p.r_position.row),
                    (p.s_position.col * 3 - p.s_position.row),
                ),
                p,
            )
        ...
        if self.legal_actions[action] == 0:
            ...
            if self.phase == 0:  # initial cyclist positioning (start with sprinter)
                ...
                if self.current_player_num == self.n_players:
                    if (
                        self.hand_number == 0
                    ):  # switch to choosing the card from the second hand
                        self.hand_number = 1
                        self.draw_cards()
                        self.current_player_num = 0
    ...
    def set_start_positions(self):
        # build cyclists list
        self.cyclists = [(p, "r") for p in self.board.players] + [
            (p, "s") for p in self.board.players
        ]
        # shuffle
                ...
                for p in self.board.players:
                    ...
                    for pen in self.penalty:
                        if p.n == int(pen[0]):
                            penalty += "X" + pen[1]
                    logger.debug(
                        f"\033[{PLAYER_COLOR_MAP[str(p.n)]}mPlayer {p.name} played : {p.r_chosen.name} {p.s_chosen.name}   Penalty: {penalty}\033[0m|          "
                    )
            else:
    ...
    def render(self, mode="human", close=False):
        ...
        if mode == "human" and not self.done:
            ...
            if self.phase == 2:
                ...
                logger.debug(
                    f"\033[{PLAYER_COLOR_MAP[str(p.n)]}mPlayer {p.name}'s {cyclist} hand\033[0m"
                )
                line = (" " * tab_size) + "".join(
                    [
                        c.name
                        + " ("
                        + str(self.from_card_to_action(c))
                        + ")"
                        + " " * len(c.name)
                        for c in p.c_hand(cyclist).cards
                    ]
                )
                logger.debug(f"{line}")
                ...
                logger.debug(
                    f"\033[{PLAYER_COLOR_MAP[str(p.n)]}mPlayer {p.name} to choose hand\033[0m"
                )
                logger.debug(
                    [
                        index
                ...
                logger.debug(
                    f"\033[{PLAYER_COLOR_MAP[str(p.n)]}mPlayer {p.name} to place cyclist\033[0m"
                )
                logger.debug(
                    [
                        index
