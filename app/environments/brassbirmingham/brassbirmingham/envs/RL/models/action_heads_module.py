...
class MultiActionHeadsGeneralised(nn.Module):
    ...
    def forward(
        ...
        if condition_on_action_type is not None:
            type_output = torch.zeros(
                1, 13, dtype=torch.float32, device=self.dummy_param.device
            )
            type_output[0, condition_on_action_type] = 1.0
            first_output = torch.tensor(
                [[condition_on_action_type]],
                dtype=torch.long,
                device=self.dummy_param.device,
            )
            first_filter = torch.zeros(
                1, 1, dtype=torch.float32, device=self.dummy_param.device
            )
        ...
        for i, head in enumerate(self.action_heads):
            ...
            for entry in self.autoregressive_map[i]:
                ...
                if entry[0] >= 0:
                    initial_input *= (
                        1 - head_log_probs_filtered[entry[0]]
                    )  # filter inputs that are masked out.
            ...
            if len(self.type_conditional_action_masks[i]):
                ...
                for (
                    prev_head_ind,
                    head_type_to_option_map,
                ) in self.type_conditional_action_masks[i].items():
                    if actions is not None:
                        masks_to_mult = masks[i][
                            head_type_to_option_map[actions[prev_head_ind]].squeeze(),
                            np.arange(main_head_inputs.size(0)),
                            :,
                        ]
                        if prev_head_ind == 4:
                            action_type_mask = (
                                actions[0] != ActionTypes.PlayDevelopmentCard
                            ).squeeze()
                        ...
                        if prev_head_ind == 4:
                            action_type_mask = (
                                action_outputs[0] != ActionTypes.PlayDevelopmentCard
                            ).squeeze()
                            masks_to_mult[action_type_mask, ...] = 1.0
                        head_mask *= masks_to_mult
            else:
            ...
            if head.returns_distribution:
                ...
                if head.type == "categorical":
                    one_hot_head_action = torch.zeros(
                        main_input.size(0),
                        head.output_dim,
                        device=self.dummy_param.device,
                    )
                ...
                if log_specific_head_probs:
                    ...
                    if i == 0:
                        ...
                        prob_to_store = np.exp(
                            head_log_prob.squeeze().cpu().data.numpy()
                        )
                        ...
                        if store_head_prob:
                            actual_action = int(
                                head_action.squeeze().cpu().data.numpy()
                            )
                            prob_to_store = np.exp(
                                head_log_prob.squeeze().cpu().data.numpy()
                            )
                ...
                if self.log_prob_masks[i] is not None:
                    for prev_head_ind, head_type_mask in self.log_prob_masks[i].items():
                        ...
                        head_prob_mask = head_type_mask[
                            acts_to_mask[prev_head_ind].squeeze()
                        ].view(-1, 1)
                        if prev_head_ind == 4:
                            action_type_mask = (
                                acts_to_mask[0] != ActionTypes.PlayDevelopmentCard
                            ).squeeze()
                ...
                head_output, action_output, head_log_prob, head_entropy = head(
                    main_head_inputs,
                    head_mask,
                    custom_inputs,
                    self.log_prob_masks[i],
                    head_log_probs_filtered,
                    prev_head_acs,
                    actions=action_inp,
...
class ActionHead(nn.Module):
    def __init__(
        ...
        if type == "categorical":
            ...
            self.distribution = DiagGaussian(
                num_inputs=dist_input_size, num_outputs=output_dim
            )
        else:
    ...
    def forward(self, main_input, mask, custom_inputs):
        if custom_inputs is not None and len(self.custom_inputs) > 0:
            input_custom = torch.cat(
                [custom_inputs[key] for key in self.custom_inputs.keys()], dim=-1
            )
