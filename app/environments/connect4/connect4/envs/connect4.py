...
class Connect4Env(gym.Env):
    ...
    def observation(self):
        if self.current_player.token.number == 1:
            position_1 = np.array(
                [1 if x.number == 1 else 0 for x in self.board]
            ).reshape(self.grid_shape)
            position_2 = np.array(
                [1 if x.number == -1 else 0 for x in self.board]
            ).reshape(self.grid_shape)
            position_3 = np.array(
                [self.can_be_placed(i) for i, x in enumerate(self.board)]
            ).reshape(self.grid_shape)
        else:
            position_1 = np.array(
                [1 if x.number == -1 else 0 for x in self.board]
    ...
    def check_game_over(self, board=None, player=None):
        ...
        for x, y, z, a in WINNERS:
            if (
                self.square_is_player(board, x, player)
                and self.square_is_player(board, y, player)
                and self.square_is_player(board, z, player)
                and self.square_is_player(board, a, player)
            ):
                return 1, True
    ...
    def rules_move(self):
        ...
        for action in range(self.action_space.n):
            if self.is_legal(action):
                ...
                if done:
                    ...
                    action_probs[action] = 1 - WRONG_MOVE_PROB * (
        ...
        action, masked_action_probs = self.sample_masked_action(
