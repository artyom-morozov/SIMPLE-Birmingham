...
class TicTacToeEnv(gym.Env):
    ...
    def observation(self):
        if self.players[self.current_player_num].token.number == 1:
            ...
            position = np.array([-x.number for x in self.board]).reshape(
                self.grid_shape
            )
    ...
    def check_game_over(self):
        ...
        for i in range(self.grid_length):
            ...
            if (
                self.square_is_player(i * self.grid_length, current_player_num)
                and self.square_is_player(i * self.grid_length + 1, current_player_num)
                and self.square_is_player(i * self.grid_length + 2, current_player_num)
            ) or (
                self.square_is_player(i + 0, current_player_num)
                and self.square_is_player(i + self.grid_length, current_player_num)
                and self.square_is_player(i + self.grid_length * 2, current_player_num)
            ):
                return 1, True
        ...
        # diagonals
        if (
            self.square_is_player(0, current_player_num)
            and self.square_is_player(4, current_player_num)
            and self.square_is_player(8, current_player_num)
        ) or (
            self.square_is_player(2, current_player_num)
            and self.square_is_player(4, current_player_num)
            and self.square_is_player(6, current_player_num)
        ):
            return 1, True
    ...
    def render(self, mode="human", close=False, verbose=True):
        ...
        logger.debug(
            " ".join(
                [x.symbol for x in self.board[self.grid_length : self.grid_length * 2]]
            )
        )
        logger.debug(
            " ".join(
                [
                    x.symbol
                    for x in self.board[(self.grid_length * 2) : (self.grid_length * 3)]
                ]
            )
        )
...
def checkWin(b, m):
    return (
        (b[0] == m and b[1] == m and b[2] == m)
        or (b[3] == m and b[4] == m and b[5] == m)  # H top
        or (b[6] == m and b[7] == m and b[8] == m)  # H mid
        or (b[0] == m and b[3] == m and b[6] == m)  # H bot
        or (b[1] == m and b[4] == m and b[7] == m)  # V left
        or (b[2] == m and b[5] == m and b[8] == m)  # V centre
        or (b[0] == m and b[4] == m and b[8] == m)  # V right
        or (b[2] == m and b[4] == m and b[6] == m)  # LR diag
    )  # RL diag
