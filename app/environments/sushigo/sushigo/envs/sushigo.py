...
class SushiGoEnv(gym.Env):
    ...
    def __init__(self, verbose=False, manual=False):
        ...
        self.observation_space = gym.spaces.Box(
            ...
            1,
            (
                self.total_cards * self.total_positions
                + self.n_players
                + self.action_space.n,
            ),
        )
    ...
    def observation(self):
        ...
        for (
            ...
            self.players
        ):  # TODO this should be from reference point of the current_player
        ...
        for i in range(len(hand)):
            ...
            if "chopsticks" in [x.type for x in self.current_player.position.cards]:
                for j in range(i + 1, len(hand)):
                    legal_actions[
                        self.card_types
                        + (hand[i].order * self.card_types)
                        + hand[j].order
                    ] = 1
                    legal_actions[
                        self.card_types
                        + (hand[j].order * self.card_types)
                        + hand[i].order
                    ] = 1
