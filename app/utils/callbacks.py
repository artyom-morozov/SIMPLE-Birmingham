...
class SelfPlayCallback(EvalCallback):
    def __init__(self, opponent_type, threshold, env_name, *args, **kwargs):
        ...
        self.generation, self.base_timesteps, pbmr, bmr = get_model_stats(
            get_best_model_name(env_name)
        )
        ...
        ):  # if evaling against rules-based agent as well, reset this too
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            result = super(
                SelfPlayCallback, self
            )._on_step()  # this will set self.best_mean_reward to the reward from the evaluation as it's previously -np.inf
            ...
            if self.callback is not None:
                rules_based_rewards = MPI.COMM_WORLD.allgather(
                    self.callback.best_mean_reward
                )
            ...
            if result and av_reward > self.threshold:
                ...
                if rank == 0:  # write new files
                    ...
                    source_file = os.path.join(
                        config.TMPMODELDIR, f"best_model.zip"
                    )  # this is constantly being written to - not actually the best model
                    target_file = os.path.join(
                        self.model_dir,
                        f"_model_{generation_str}_{av_rules_based_reward_str}_{av_rewards_str}_{str(self.base_timesteps + self.num_timesteps)}_.zip",
                    )
