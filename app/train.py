...
def main(args):
    ...
    env = selfplay_wrapper(base_env)(
        opponent_type=args.opponent_type, verbose=args.verbose
    )
    env.seed(workerseed)
    ...
    callback_args = {
        "eval_env": selfplay_wrapper(base_env)(
            opponent_type=args.opponent_type, verbose=args.verbose
        ),
    ...
    if args.rules:
        ...
        eval_actual_callback = EvalCallback(
            eval_env=selfplay_wrapper(base_env)(
                opponent_type="rules", verbose=args.verbose
            ),
            eval_freq=1,
            n_eval_episodes=args.n_eval_episodes,
    ...
    eval_callback = SelfPlayCallback(
        args.opponent_type, args.threshold, args.env_name, **callback_args
    )
...
def cli() -> None:
    ...
    parser.add_argument(
        "--gamma", "-g", type=float, default=0.99, help="The value of gamma in PPO"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--clip_param", "-c", type=float, default=0.2, help="The clip paramater in PPO"
    )
    parser.add_argument(
        "--entcoeff",
        "-ent",
    ...
    parser.add_argument(
        "--lam", "-l", type=float, default=0.95, help="The value of lambda in PPO"
    )
    parser.add_argument(
        "--adam_epsilon",
        "-a",
